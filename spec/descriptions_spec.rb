# frozen_string_literal: true

require 'spec_helper'

require 'folding_at_home_client/description'
require 'folding_at_home_client/descriptions'

RSpec.describe FoldingAtHomeClient::Descriptions do
  describe '.all' do
    let(:endpoint) { '/project/description' }
    let(:description_instances) do
      [
        instance_double(FoldingAtHomeClient::Description),
        instance_double(FoldingAtHomeClient::Description),
      ]
    end

    before do
      allow(described_class).to receive(:request_and_instantiate_objects)
        .with(endpoint:, object_class: FoldingAtHomeClient::Description)
        .and_return(description_instances)
    end

    it 'calls request_and_instantiate_objects with the correct endpoint and object class' do
      described_class.all
      expect(described_class).to have_received(:request_and_instantiate_objects)
        .with(endpoint:, object_class: FoldingAtHomeClient::Description)
    end

    it 'returns the instances created by request_and_instantiate_objects' do
      result = described_class.all
      expect(result).to eq(description_instances)
    end
  end
end
